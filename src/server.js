...
const onJoined = (sock) => {
	...
	socket.on('join', (data) => {
		...
		var online = (Object.keys(users).length < 2) ?
...
const ping = (socket) => {
	...
	var iv = setInterval(function(){
		...
		users[sock.name].tm = setTimeout(function (){
			clearInterval(iv);
...
const onMsg = (socket) => {
	...
	sock.on('msgToServer', (data) => {
		if(data.msg[0] === '/'){
			let command = data.msg.split(' ')[0];
			let info = data.msg.substring(command.length+1);
			switch(command){
				case '/aboutme':
				...
				case '/whisper':
					...
					let whisper = info.substring(username.length+1);
